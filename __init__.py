...
class MkzUrbanDemo(MycroftSkill):
    ...
    def _route_new(self, message):
        ...
        if (self.gui["routeNext"]):
            if (self.gui["routeTimeToNext"]>=2):
                self.schedule_event(self._route_next_instruction, self.gui["routeTimeToNext"]/2)
            else:
    ...
    def _route_position(self, message):
        ...
        self.log.info("segment: %d(%d) / path: %d",message.data["segment"],message.data["segments"],message.data["path"])
        if (self.gui["routeNext"]\
